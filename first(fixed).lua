...
_G._MassAttackPro = ST          -- expose for future cleanup
...
----------------------------------------------------------------
-- 6. COLOR PALETTE  (module-level so loops can reference it)
----------------------------------------------------------------
local CLR = {
...
-- 9. GUI
----------------------------------------------------------------
local GUI_REFS  -- forward declare; set after creation
...
local function BuildGUI()
    ...
    local function Stroke(p, col, t)
        local s = Instance.new("UIStroke"); s.Color = col or CLR.border; s.Thickness = t or 1; s.Parent = p
    end

    -- ── main frame ──
    local main = Instance.new("Frame")
    main.Name = "Main"
    main.Size     = UDim2.new(0, 370, 0, 510)
    main.Position = UDim2.new(0.5, -185, 0.5, -255)
    ...
    sh.Size = UDim2.new(1,40,1,40); sh.Position = UDim2.new(0,-20,0,-20)
    ...
    hdr.Name = "Header"; hdr.Size = UDim2.new(1,0,0,44)
    hdr.BackgroundColor3 = CLR.header; hdr.BorderSizePixel = 0; hdr.Parent = main
    Corner(hdr, 10)
    -- fix bottom corners
    local hf = Instance.new("Frame")
    hf.Size = UDim2.new(1,0,0,12); hf.Position = UDim2.new(0,0,1,-12)
    hf.BackgroundColor3 = CLR.header; hf.BorderSizePixel = 0; hf.Parent = hdr
    -- separator
    local sep = Instance.new("Frame")
    sep.Size = UDim2.new(1,0,0,1); sep.Position = UDim2.new(0,0,1,0)
    sep.BackgroundColor3 = CLR.border; sep.BorderSizePixel = 0; sep.Parent = hdr

    -- title
    local ttl = Instance.new("TextLabel")
    ttl.Size = UDim2.new(1,-110,1,0); ttl.Position = UDim2.new(0,12,0,0)
    ttl.BackgroundTransparency = 1; ttl.Text = "⚔  MASS ATTACK PRO"
    ...
    vb.Size = UDim2.new(0,36,0,16); vb.Position = UDim2.new(0,176,0.5,-8)
    vb.BackgroundColor3 = CLR.accent; vb.Text = "v2.0"
    vb.TextColor3 = CLR.text; vb.TextSize = 9; vb.Font = Enum.Font.GothamBold; vb.Parent = hdr
    Corner(vb, 5)

    -- minimize
    local minB = Instance.new("TextButton")
    minB.Size = UDim2.new(0,26,0,26); minB.Position = UDim2.new(1,-64,0.5,-13)
    minB.BackgroundColor3 = CLR.orange; minB.Text = "—"
    minB.TextColor3 = CLR.text; minB.TextSize = 14; minB.Font = Enum.Font.GothamBold
    minB.AutoButtonColor = true; minB.Parent = hdr; Corner(minB, 6)

    -- close
    local clsB = Instance.new("TextButton")
    clsB.Size = UDim2.new(0,26,0,26); clsB.Position = UDim2.new(1,-34,0.5,-13)
    clsB.BackgroundColor3 = CLR.red; clsB.Text = "✕"
    clsB.TextColor3 = CLR.text; clsB.TextSize = 11; clsB.Font = Enum.Font.GothamBold
    ...
    do
        ...
        Conn(hdr.InputChanged, function(inp)
            ...
            or inp.UserInputType == Enum.UserInputType.Touch then dInput = inp end
        end)
    ...
    scroll.ScrollBarThickness = 3; scroll.ScrollBarImageColor3 = CLR.accent
    ...
    local function Section(name)
        ...
        l.TextColor3 = CLR.accent; l.TextSize = 10; l.Font = Enum.Font.GothamBold
        l.TextXAlignment = Enum.TextXAlignment.Left; l.Parent = f

        local ln = Instance.new("Frame")
        ln.Size = UDim2.new(1,-8,0,1); ln.Position = UDim2.new(0,4,1,-1)
        ln.BackgroundColor3 = CLR.border; ln.BorderSizePixel = 0; ln.Parent = f
    end

    -- ── toggle builder ──
    local function Toggle(label, def, cb)
        local f = Instance.new("Frame")
        f.Size = UDim2.new(1,0,0,32); f.BackgroundColor3 = CLR.card
        f.BorderSizePixel = 0; f.LayoutOrder = nxt(); f.Parent = scroll
        Corner(f, 7)

        local tl = Instance.new("TextLabel")
        tl.Size = UDim2.new(1,-62,1,0); tl.Position = UDim2.new(0,10,0,0)
        tl.BackgroundTransparency = 1; tl.Text = label
        tl.TextColor3 = CLR.text; tl.TextSize = 12; tl.Font = Enum.Font.Gotham
        tl.TextXAlignment = Enum.TextXAlignment.Left
        tl.TextTruncate = Enum.TextTruncate.AtEnd; tl.Parent = f

        local bg = Instance.new("Frame")
        bg.Size = UDim2.new(0,40,0,18); bg.Position = UDim2.new(1,-50,0.5,-9)
        ...
        function api.Set(v)
            ...
            TweenService:Create(dot, TweenInfo.new(0.14, Enum.EasingStyle.Back),
                {Position = st and UDim2.new(1,-16,0.5,-7) or UDim2.new(0,2,0.5,-7)}):Play()
        end
    ...
    local function Slider(label, lo, hi, def, cb)
        local f = Instance.new("Frame")
        f.Size = UDim2.new(1,0,0,46); f.BackgroundColor3 = CLR.card
        f.BorderSizePixel = 0; f.LayoutOrder = nxt(); f.Parent = scroll
        Corner(f, 7)

        local tl = Instance.new("TextLabel")
        tl.Size = UDim2.new(1,-52,0,18); tl.Position = UDim2.new(0,10,0,3)
        tl.BackgroundTransparency = 1; tl.Text = label
        tl.TextColor3 = CLR.text; tl.TextSize = 11; tl.Font = Enum.Font.Gotham
        tl.TextXAlignment = Enum.TextXAlignment.Left; tl.Parent = f

        local vl = Instance.new("TextLabel")
        vl.Size = UDim2.new(0,42,0,18); vl.Position = UDim2.new(1,-52,0,3)
        vl.BackgroundTransparency = 1; vl.Text = tostring(def)
        vl.TextColor3 = CLR.accent; vl.TextSize = 12; vl.Font = Enum.Font.GothamBold
        ...
        track.BackgroundColor3 = CLR.sliderBg; track.BorderSizePixel = 0
        track.Parent = f; Corner(track, 3)

        local fill = Instance.new("Frame")
        fill.Size = UDim2.new(math.clamp((def-lo)/(hi-lo),0,1),0,1,0)
        fill.BackgroundColor3 = CLR.accent; fill.BorderSizePixel = 0
        fill.Parent = track; Corner(fill, 3)

        local knob = Instance.new("Frame")
        knob.Size = UDim2.new(0,12,0,12)
        knob.Position = UDim2.new(math.clamp((def-lo)/(hi-lo),0,1),-6,0.5,-6)
        knob.BackgroundColor3 = Color3.new(1,1,1); knob.BorderSizePixel = 0
        knob.ZIndex = 2; knob.Parent = track; Corner(knob, 6)

        local hit = Instance.new("TextButton")
        hit.Size = UDim2.new(1,0,0,20); hit.Position = UDim2.new(0,0,0,22)
        ...
        return { Set = function(v)
            cur = v; local r = math.clamp((v-lo)/(hi-lo),0,1)
            fill.Size = UDim2.new(r,0,1,0)
            knob.Position = UDim2.new(r,-6,0.5,-6)
            vl.Text = tostring(v)
        end, Get = function() return cur end }
    end
    ...
    sCard.Size = UDim2.new(1,0,0,88); sCard.BackgroundColor3 = CLR.card
    sCard.BorderSizePixel = 0; sCard.LayoutOrder = nxt(); sCard.Parent = scroll
    Corner(sCard, 7); Stroke(sCard)

    local sDot = Instance.new("Frame")
    sDot.Size = UDim2.new(0,8,0,8); sDot.Position = UDim2.new(0,10,0,11)
    sDot.BackgroundColor3 = CLR.red; sDot.BorderSizePixel = 0; sDot.Parent = sCard
    Corner(sDot, 4)

    local sLbl = Instance.new("TextLabel")
    sLbl.Size = UDim2.new(0.4,0,0,16); sLbl.Position = UDim2.new(0,24,0,6)
    sLbl.BackgroundTransparency = 1; sLbl.Text = "DISABLED"
    sLbl.TextColor3 = CLR.red; sLbl.TextSize = 11; sLbl.Font = Enum.Font.GothamBold
    sLbl.TextXAlignment = Enum.TextXAlignment.Left; sLbl.Parent = sCard

    local function StatL(name, y)
        local n = Instance.new("TextLabel")
        n.Size = UDim2.new(0,62,0,14); n.Position = UDim2.new(0,10,0,y)
        n.BackgroundTransparency = 1; n.Text = name..":"
        n.TextColor3 = CLR.textDim; n.TextSize = 10; n.Font = Enum.Font.Gotham
        n.TextXAlignment = Enum.TextXAlignment.Left; n.Parent = sCard
        local v = Instance.new("TextLabel")
        v.Size = UDim2.new(0,50,0,14); v.Position = UDim2.new(0,72,0,y)
        v.BackgroundTransparency = 1; v.Text = "0"
        v.TextColor3 = CLR.text; v.TextSize = 10; v.Font = Enum.Font.GothamBold
        v.TextXAlignment = Enum.TextXAlignment.Left; v.Parent = sCard
        return v
    end
    local function StatR(name, y)
        local n = Instance.new("TextLabel")
        n.Size = UDim2.new(0,50,0,14); n.Position = UDim2.new(0.5,8,0,y)
        n.BackgroundTransparency = 1; n.Text = name..":"
        n.TextColor3 = CLR.textDim; n.TextSize = 10; n.Font = Enum.Font.Gotham
        n.TextXAlignment = Enum.TextXAlignment.Left; n.Parent = sCard
        local v = Instance.new("TextLabel")
        v.Size = UDim2.new(0.5,-66,0,14); v.Position = UDim2.new(0.5,58,0,y)
    ...
    Toggle("Punch "   .. (REM.Punch    and "✓" or "✗"), REM.Punch    ~= nil and CFG.UsePunch,    function(v) if REM.Punch    then CFG.UsePunch    = v end end)
    Toggle("Suplex "  .. (REM.Suplex   and "✓" or "✗"), REM.Suplex   ~= nil and CFG.UseSuplex,   function(v) if REM.Suplex   then CFG.UseSuplex   = v end end)
    Toggle("Heavy Hit ".. (REM.HeavyHit and "✓" or "✗"), REM.HeavyHit ~= nil and CFG.UseHeavyHit, function(v) if REM.HeavyHit then CFG.UseHeavyHit = v end end)

    Section("Defense")

    local tGuard = Toggle("Auto Guard " .. (REM.Block and "✓" or "✗"), REM.Block ~= nil and CFG.UseAutoGuard, function(v)
        if REM.Block then CFG.UseAutoGuard = v; if not v then SetGuard(false) end end
    end)

    Toggle("Drop Guard to Attack", CFG.GuardDropForAttack, function(v) CFG.GuardDropForAttack = v end)
    Toggle("Ignore ForceField",    CFG.IgnoreForceField,   function(v) CFG.IgnoreForceField   = v end)
    ...
    ri.Size = UDim2.new(1,0,0,74); ri.BackgroundColor3 = CLR.card
    ri.BorderSizePixel = 0; ri.LayoutOrder = nxt(); ri.Parent = scroll; Corner(ri, 7)
    ...
    for idx, data in ipairs({
        ...
        rl.Size = UDim2.new(1,-14,0,15); rl.Position = UDim2.new(0,10,0,2+(idx-1)*17)
    ...
    kb.Size = UDim2.new(1,0,0,18); kb.BackgroundTransparency = 1
    kb.Text = "Insert → Toggle GUI   |   RShift → Toggle Script"
    kb.TextColor3 = CLR.textDim; kb.TextSize = 9; kb.Font = Enum.Font.Gotham
    ...
    clsB.MouseButton1Click:Connect(function()
        ST.Running = false; CFG.Enabled = false; SetGuard(false); sg:Destroy()
    end)
...
task.spawn(function()
    while ST.Running do
        if CFG.UseAutoGuard and CFG.Enabled and not ST.GuardActive then
            SetGuard(true)
        end
        task.wait(0.4)
    end
...
task.spawn(function()
    while ST.Running do
        if CFG.Enabled and HAS_ATTACK then
            ...
            local wait = interval - (tick() - t0)
            task.wait(wait > 0 and wait or 0.001)
        else
            task.wait(0.2)
        end
    end
...
task.spawn(function()
    while ST.Running do
        if GUI_REFS and GUI_REFS.sv then
            ...
            s.health.Text  = humanoid and tostring(math.floor(humanoid.Health)) or "?"
...
----------------------------------------------------------------
RefreshTargets()
if CFG.UseAutoGuard and CFG.Enabled then SetGuard(true) end

print("╔═══════════════════════════════════════╗")
print("║     MASS ATTACK PRO v2.0 — LOADED     ║")
print("╠═══════════════════════════════════════╣")
