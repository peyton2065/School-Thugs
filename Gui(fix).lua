...
local REM = {
    ...
    -- Admin (detection only for status display)
...
----------------------------------------------------------------
-- 8b. GAME SCANNER (food items + weapon spawns)
----------------------------------------------------------------
local FOOD_KEYWORDS = {
    "food", "pizza", "burger", "meat", "steak", "chicken",
    "sandwich", "hotdog", "taco", "burrito", "soda", "drink",
    "water", "juice", "milk", "apple", "bread", "donut",
    "candy", "chips", "snack", "fries", "rice", "soup",
    "heal", "health", "potion", "bandage", "medkit", "med",
    "carne", "torta", "empanada", "elote", "churro",
}

local WEAPON_KEYWORDS = {
    "bat", "metal", "weapon", "knife", "sword", "pipe",
    "gun", "pistol", "rifle", "shotgun", "tool", "crowbar",
    "machete", "hammer", "axe", "stick", "club", "chain",
    "wrench", "bottle", "brick", "rock", "pan", "shovel",
}
...
-- Scan the game for food item references (names the Carniceria remote might accept)
local function ScanForFoodItems()
    local items = {}
    local checked = {}

    -- Scan ReplicatedStorage for food-related values/folders
    pcall(function()
        for _, obj in ipairs(RepStorage:GetDescendants()) do
            ...
            -- Check StringValues that might list food
            if obj:IsA("StringValue") and IsLikelyFood(obj.Value) then
    ...
    -- Scan workspace for food models/tools
    pcall(function()
        for _, obj in ipairs(workspace:GetDescendants()) do
            if (obj:IsA("Tool") or obj:IsA("Model")) and not checked[obj.Name] then
                if IsLikelyFood(obj.Name) then
                    checked[obj.Name] = true
                    table.insert(items, obj.Name)
                end
            end
    ...
    -- Scan player backpack for food tools
    pcall(function()
        local backpack = plr:FindFirstChild("Backpack")
        if backpack then
            for _, tool in ipairs(backpack:GetChildren()) do
                if tool:IsA("Tool") and not checked[tool.Name] then
                    if IsLikelyFood(tool.Name) then
    ...
    -- Scan StarterPack for default tools
    pcall(function()
    ...
    -- Scan for shop GUIs that might list items
    pcall(function()
        local pg = plr:FindFirstChild("PlayerGui")
        if pg then
            for _, gui in ipairs(pg:GetDescendants()) do
                if gui:IsA("TextLabel") or gui:IsA("TextButton") then
                    if gui.Text and gui.Text ~= "" and not checked[gui.Text] then
                        if IsLikelyFood(gui.Text) then
                            checked[gui.Text] = true
                            table.insert(items, gui.Text)
                        end
                    end
                end
            end
    ...
    ST.FoodItems = items
    Log("Food scan found", #items, "items:", table.concat(items, ", "))
    return items
end

-- Scan workspace for pickupable weapon spawns
local function ScanForWeaponSpawns()
    local weapons = {}
    if not rootPart or not rootPart.Parent then return weapons end
    local pos = rootPart.Position

    -- Method 1: Tools directly in workspace
    pcall(function()
    ...
    -- Method 2: Tools inside models in workspace
    pcall(function()
        for _, obj in ipairs(workspace:GetDescendants()) do
    ...
    -- Method 3: Models that look like weapons (not Tool class)
    pcall(function()
        for _, obj in ipairs(workspace:GetChildren()) do
            if obj:IsA("Model") and IsLikelyWeapon(obj.Name) then
    ...
    -- Method 4: BaseParts that look like weapons (standalone parts)
    pcall(function()
        for _, obj in ipairs(workspace:GetChildren()) do
            if obj:IsA("BasePart") and IsLikelyWeapon(obj.Name) then
    ...
    -- Method 5: Check specific game folders
    pcall(function()
        local possibleFolders = {
            ...
            workspace:FindFirstChild("DropItems"),
    ...
    table.sort(weapons, function(a, b) return a.dist < b.dist end)

    ST.WeaponSpawns = weapons
    Log("Weapon scan found", #weapons, "items")
    return weapons
end
...
local function Attack(target)
    ...
    -- Weapon hit alongside fists when not preferring weapon
...
local function TryBuyFood()
    ...
    Log("Carniceria: attempting purchase")

    -- Rescan food items periodically
    if #ST.FoodItems == 0 or (now - ST.StartTime) % 30 < 1 then
        ScanForFoodItems()
    end

    -- Strategy 1: Fire with no args (some shops just give you food)
    SafeCall(function() REM.Carniceria:FireServer() end)

    -- Strategy 2: Fire with boolean/action
    SafeCall(function() REM.Carniceria:FireServer(true) end)
    SafeCall(function() REM.Carniceria:FireServer("Buy") end)
    SafeCall(function() REM.Carniceria:FireServer("Purchase") end)
    SafeCall(function() REM.Carniceria:FireServer("Comprar") end)
    SafeCall(function() REM.Carniceria:FireServer("Use") end)
    SafeCall(function() REM.Carniceria:FireServer("Eat") end)
    SafeCall(function() REM.Carniceria:FireServer("Heal") end)

    -- Strategy 3: Fire with numeric indices (item slot 1, 2, 3...)
    for i = 1, 10 do
        SafeCall(function() REM.Carniceria:FireServer(i) end)
    end
    ...
    for _, foodName in ipairs(commonFoods) do
        SafeCall(function() REM.Carniceria:FireServer(foodName) end)
        SafeCall(function() REM.Carniceria:FireServer("Buy", foodName) end)
    end

    -- Strategy 5: Use any food items found by the scanner
    for _, itemName in ipairs(ST.FoodItems) do
        SafeCall(function() REM.Carniceria:FireServer(itemName) end)
        SafeCall(function() REM.Carniceria:FireServer("Buy", itemName) end)
        SafeCall(function() REM.Carniceria:FireServer(itemName, 1) end)
    end

    -- Strategy 6: Fire with player reference (some shops need to know who's buying)
    SafeCall(function() REM.Carniceria:FireServer(plr) end)
    SafeCall(function() REM.Carniceria:FireServer(plr, "Buy") end)
    SafeCall(function() REM.Carniceria:FireServer(plr, 1) end)
    SafeCall(function() REM.Carniceria:FireServer(plr.Name) end)

    -- Strategy 7: Fire with character reference
    SafeCall(function() REM.Carniceria:FireServer(char) end)
    SafeCall(function() REM.Carniceria:FireServer(char, "Heal") end)

    -- Strategy 8: Table argument (some remotes expect structured data)
    SafeCall(function() REM.Carniceria:FireServer({Action = "Buy"}) end)
    SafeCall(function() REM.Carniceria:FireServer({Item = "Food", Amount = 1}) end)
    SafeCall(function() REM.Carniceria:FireServer({Type = "Heal"}) end)
    SafeCall(function() REM.Carniceria:FireServer({Player = plr, Action = "Buy"}) end)

    ST.TotalFoodBuys = ST.TotalFoodBuys + 1
    ST.LastFoodBuy = now
    Log("Carniceria: purchase volley fired (" .. ST.TotalFoodBuys .. " total)")
end

-- 9k  Pickup Tools (remote only) — FIXED
local function TryPickupTools()
    ...
    -- Scan for nearby weapons
    local weapons = ScanForWeaponSpawns()

    if #weapons == 0 then return end

    -- Try to pick up the nearest weapon
    local target = weapons[1]
    Log("Pickup: attempting", target.name, "type:", target.type, "dist:", math.floor(target.dist))

    -- Fire with the instance itself
    SafeCall(function() REM.PickupTool:FireServer(target.instance) end)

    -- Fire with the instance name
    SafeCall(function() REM.PickupTool:FireServer(target.name) end)

    -- Fire with the part/handle
    SafeCall(function() REM.PickupTool:FireServer(target.part) end)

    -- Fire with instance + part
    SafeCall(function() REM.PickupTool:FireServer(target.instance, target.part) end)

    -- Fire with just no args (maybe proximity-based)
    SafeCall(function() REM.PickupTool:FireServer() end)

    -- Fire with boolean
    SafeCall(function() REM.PickupTool:FireServer(true) end)

    -- Fire with player + item
    SafeCall(function() REM.PickupTool:FireServer(plr, target.instance) end)
    SafeCall(function() REM.PickupTool:FireServer(plr, target.name) end)

    -- Fire with character + item
    SafeCall(function() REM.PickupTool:FireServer(char, target.instance) end)

    -- Fire with name as action
    SafeCall(function() REM.PickupTool:FireServer("Pickup", target.instance) end)
    SafeCall(function() REM.PickupTool:FireServer("Pickup", target.name) end)
    SafeCall(function() REM.PickupTool:FireServer("Equip", target.instance) end)
    SafeCall(function() REM.PickupTool:FireServer("Equip", target.name) end)
    SafeCall(function() REM.PickupTool:FireServer("Grab", target.instance) end)

    -- Fire with table
    SafeCall(function() REM.PickupTool:FireServer({Tool = target.instance}) end)
    SafeCall(function() REM.PickupTool:FireServer({Item = target.name}) end)
    SafeCall(function() REM.PickupTool:FireServer({Tool = target.name, Player = plr}) end)

    -- If there's a parent model, try that too
    if target.instance.Parent and target.instance.Parent ~= workspace then
        SafeCall(function() REM.PickupTool:FireServer(target.instance.Parent) end)
        SafeCall(function() REM.PickupTool:FireServer(target.instance.Parent.Name) end)
    end

    -- Try every weapon found (not just nearest)
    if #weapons > 1 then
        for i = 2, math.min(#weapons, 5) do
            local w = weapons[i]
            SafeCall(function() REM.PickupTool:FireServer(w.instance) end)
            SafeCall(function() REM.PickupTool:FireServer(w.name) end)
            SafeCall(function() REM.PickupTool:FireServer(w.part) end)
        end
    end

    ST.TotalPickups = ST.TotalPickups + 1
    ST.LastToolPickup = now
    Log("Pickup: volley fired for", target.name, "(", ST.TotalPickups, "attempts total)")
end
...
local function BuildGUI()
    ...
    Slider("Heal Threshold (%)", 10, 100, CFG.HealThreshold, function(v)
    ...
    Toggle("Smart PVP Toggle " .. (REM.PVPToggle and "✓" or "✗"),
        REM.PVPToggle ~= nil and CFG.UseSmartPVP,
    ...
    Toggle("Auto Pickup Weapons " .. (REM.PickupTool and "✓" or "✗"),
        CFG.UseAutoPickupTools,
        function(v) if REM.PickupTool then CFG.UseAutoPickupTools = v end; AutoSave() end)

    Slider("Pickup Range (studs)", 10, 200, CFG.ToolPickupRange, function(v)
    ...
    Toggle("Auto Guard " .. (REM.Block and "✓" or "✗"),
    ...
    Slider("Guard Range (studs)", 5, 150, CFG.GuardActivationRange, function(v)
...
-- 11f  Auto Heal + Buy Food loop
task.spawn(function()
    while ST.Running do
        if CFG.Enabled then
            if CFG.UseAutoBuyFood then
                SafeCall(TryBuyFood)
            end
            if CFG.UseAutoHeal then
                SafeCall(TryHeal)
            end
