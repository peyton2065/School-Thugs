...
local function RunKillAll()
    ...
    while CFG.KillAllActive and CFG.Enabled and ST.Running do
        ...
        for i, entry in ipairs(targets) do
            ...
            -- Track health for DPS
            local prevTargetHP = tHum.Health
            ...
            if rootPart and rootPart.Parent and tRoot and tRoot.Parent then
                ...
                if postDist > CFG.KillAllReteleportDist then
                    for _ = 1, CFG.KillAllRetries do
                        rootPart.CFrame = tRoot.CFrame * CFrame.new(0, 0, 3)
                        task.wait(0.1)
                        if (rootPart.Position - tRoot.Position).Magnitude <= CFG.KillAllReteleportDist then
                            break
                        end
                    end
                end
            ...
            while CFG.KillAllActive and CFG.Enabled and ST.Running do
                ...
                task.defer(function()
                    if tHum and tHum.Parent then
                        local dmg = hpBefore - tHum.Health
                        if dmg > 0 then RecordDamage(dmg) end
                    end
                end)
            ...
            if killed then
                ST.KillAllKills += 1
                Notify("Killed " .. target.Name .. " (" .. ST.KillAllKills .. ")", "success", 2.5)
...
task.delay(0.5, function()
    ...
    Notify("MAP v" .. SCRIPT_VERSION .. " loaded | "
           .. EXECUTOR_NAME .. " | " .. remoteCount .. "/17 remotes", "success", 4)
