                    ...
                    for _, entry in ipairs(ST.SortedTargetCache) do
                        ...
                        if not model or not model.Parent then
                            ...
                            if hrp then
                                ...
                                -- Fix 1: teleport to target before firing remotes.
                                -- The server validates proximity, so we must be within range.
                                -- Only move if we're actually out of reliable hit range (7 studs).
                                if dist > 7 then
                                    GetChar()
                                    if rootPart and rootPart.Parent then
                                        SafeTeleport(hrp.CFrame * CFrame.new(0, 0, 3))
                                        InvalidateNearbyCache()
                                        task.wait(0.03) -- brief settle so CFrame registers server-side
                                    end
                                end
